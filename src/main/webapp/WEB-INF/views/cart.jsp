	...
	<div class="wrap">
		...
		<div class="top_area">
			...
			<div class="content_area">
				<div class="content_subject"><span>CART</span></div>
				<div class="line"></div>
				<!-- 장바구니 리스트 -->
							...
							<c:forEach items="${cartInfo}" var="ci">
								<tr>
									<td class="td_width_1 cart_info_td">										
										...
										<input type="hidden" class="individual_goodsCount_input" value="${ci.goodsCount}">
										<input type="hidden" class="individual_totalPrice_input" value="${ci.goodsPrice * ci.goodsCount}">
										<input type="hidden" class="individual_point_input" value="${ci.point}">
										<input type="hidden" class="individual_totalPoint_input" value="${ci.totalPoint}">
										<input type="hidden" class="individual_goodsId_input" value="${ci.goodsId }">
									</td>
									<td class="td_width_2">
										<div class="image_wrap" data-goodscode="${ci.imageList[0].goodsCode}" data-path="${ci.imageList[0].uploadPath}" 
											data-uuid="${ci.imageList[0].uuid}" data-filename="${ci.imageList[0].fileName}">
											<img>
										</div>
									</td>
									<td class="td_width_3">${ci.goodsName}</td>
									<td class="td_width_4 price_td">
										판매가 : <span class="red_color"><fmt:formatNumber value="${ci.goodsPrice}" pattern="#,### 원"/></span><br>
										마일리지 : <span class="green_color"><fmt:formatNumber value="${ci.point}" pattern="#,###" /></span>									
									</td>
									<td class="td_width_4 table_text_align_center">
										<div class="table_text_align_center quantity_div">
											<input type="text" value="${ci.goodsCount}" class="quantity_input">
									...
									<td class="td_width_4 table_text_align_center">
										<fmt:formatNumber value="${ci.goodsPrice * ci.goodsCount}" pattern="#,### 원" />
									</td>
									<td class="td_width_4 table_text_align_center">
...
$(".individual_cart_checkbox").on("change", function(){
	/* 총 주문 정보 세팅(배송비, 총 가격, 포인트, 물품 수, 종류)*/
...
$(".quantity_modify_btn").on("click", function(){
	...
	let goodsCount = $(this).parent("td").find("input").val();
	$(".update_cartId").val(cartId);
	$(".update_goodsCount").val(goodsCount);
...
$(".order_btn").on("click", function(){
	...
	$(".cart_info_td").each(function(index, element ){
		...
		let goodsId_input = "<input name ='orders["+orderNumber+"].goodsCode' type='hidden' value='" + goodsId + "'>";
		form_contents += goodsId_input;
		
		let goodsCount_input = "<input name='orders[" + orderNumber+"].goodsCount' type='hidden' value='" +goodsCount+"'>";
